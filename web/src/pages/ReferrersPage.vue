  ...
  <section class="analytics-stack">
    <article class="panel analytics-hero">
      <div class="panel-head analytics-head">
        <div>
          <h3>{{ app.t("sourceMix") }}</h3>
          <p>{{ app.t("sourceMixText") }}</p>
        </div>
      </div>
      <div class="insight-band">
        <div v-for="item in app.referrerHighlights" :key="item.key" class="insight-pill">
    ...
    <div class="analytics-grid analytics-grid-split">
      ...
      <article class="panel">
        <div class="panel-head">
          <div>
            <h3>{{ app.t("sourceRevenue") }}</h3>
            <p>{{ app.t("revenueSignals") }}</p>
          </div>
        </div>
        <DataBars :rows="app.state.referrers" label-key="referrer" value-key="revenue" :empty-text="app.t('noData')" />
      </article>
    </div>
    ...
    <article class="panel">
      <div class="panel-head">
        <div>
          <h3>{{ app.t("sourceTable") }}</h3>
          <p>{{ app.t("quickRead") }}</p>
        </div>
      </div>
      <label class="analytics-filter">
        <span>{{ app.t("filterRows") }}</span>
        <input v-model.trim="app.referrerFilter.query" :placeholder="app.t('referrer')" />
      </label>
      <SimpleTable :rows="app.filteredReferrers" :columns="app.referrerColumns" :empty-text="app.t('noData')" />
    </article>
  </section>
</template>
